...
const getMockResponse = (type: 'text' | 'image', prompt: string) => {
  ...
  if (lowerPrompt.includes("instagram") || lowerPrompt.includes("post") || lowerPrompt.includes("marketing")) {
    return "Estratégia comprovada: Poste fotos do processo (bastidores) nos stories e o resultado final no feed. Use hashtags locais como #trancista[SuaCidade]. A frequência de postagem faz a diferença!";
  }
  ...
  if (isMentor) return "Estou aqui para te ajudar a resolver. Empreender é um desafio contínuo, mas você tem talento de sobra. Me conta: qual é a demanda mais urgente agora?";
