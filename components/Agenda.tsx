...
const Agenda = () => {
  ...
  // Initialize appointments - Force reset with version key
  const MOCK_VERSION = 'v3'; // Increment to force reset
  useEffect(() => {
    const savedVersion = localStorage.getItem('appointments_version');
    const saved = localStorage.getItem('appointments');

    // Force reset if version changed or no data
    if (savedVersion !== MOCK_VERSION || !saved) {
      ...
      const nextWeek2 = new Date(today); nextWeek2.setDate(nextWeek2.getDate() + 8);

      const mocks: Appointment[] = [
        // HOJE - 3 agendamentos
        { id: '1', clientName: 'Mariana Silva', clientPhone: '11999999999', service: 'Box Braids', date: formatDateKey(today), time: '09:00', notes: 'Cliente quer pontas finas', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },
        { id: '2', clientName: 'Ana Paula', clientPhone: '11888888888', service: 'Nagô Geométrica', date: formatDateKey(today), time: '14:30', notes: '', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },
        { id: '3', clientName: 'Fernanda Lima', clientPhone: '11777777771', service: 'Knotless', date: formatDateKey(today), time: '17:00', notes: 'Primeira vez', status: 'pending', origin: 'public_link', createdAt: new Date().toISOString() },

        // AMANHÃ - 2 agendamentos
        { id: '4', clientName: 'Juliana Guedes', clientPhone: '11777777777', service: 'Knotless', date: formatDateKey(tomorrow), time: '10:00', notes: 'Trazer jumbo extra', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },
        { id: '5', clientName: 'Camila Rocha', clientPhone: '11666666661', service: 'Twist Senegalês', date: formatDateKey(tomorrow), time: '15:00', notes: '', status: 'confirmed', origin: 'public_link', createdAt: new Date().toISOString() },

        // DEPOIS DE AMANHÃ - 3 agendamentos (dia cheio)
        { id: '6', clientName: 'Priscila Costa', clientPhone: '11666666666', service: 'Fulani Braids', date: formatDateKey(dayAfter), time: '09:00', notes: '', status: 'confirmed', origin: 'public_link', createdAt: new Date().toISOString() },
        { id: '7', clientName: 'Letícia Alves', clientPhone: '11555555551', service: 'Box Braids', date: formatDateKey(dayAfter), time: '13:00', notes: 'Cabelo curto', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },
        { id: '8', clientName: 'Raquel Mendes', clientPhone: '11444444441', service: 'Gypsy Braids', date: formatDateKey(dayAfter), time: '17:30', notes: '', status: 'pending', origin: 'public_link', createdAt: new Date().toISOString() },

        // DIA 3 - 1 agendamento
        { id: '9', clientName: 'Patrícia Souza', clientPhone: '11333333331', service: 'Goddess Locs', date: formatDateKey(day3), time: '11:00', notes: 'Retoque', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },

        // DIA 4 - 2 agendamentos
        { id: '10', clientName: 'Aline Ferreira', clientPhone: '11222222221', service: 'Passion Twist', date: formatDateKey(day4), time: '10:00', notes: '', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },
        { id: '11', clientName: 'Débora Nascimento', clientPhone: '11111111111', service: 'Nagô Geométrica', date: formatDateKey(day4), time: '15:00', notes: 'Traz referência', status: 'pending', origin: 'public_link', createdAt: new Date().toISOString() },

        // PRÓXIMA SEMANA - 2 agendamentos
        { id: '12', clientName: 'Beatriz Santos', clientPhone: '11555555555', service: 'Gypsy Braids', date: formatDateKey(nextWeek), time: '09:00', notes: 'Confirmar cor do cabelo', status: 'confirmed', origin: 'public_link', createdAt: new Date().toISOString() },
        { id: '13', clientName: 'Vanessa Oliveira', clientPhone: '11000000001', service: 'Twist Senegalês', date: formatDateKey(nextWeek2), time: '14:00', notes: '', status: 'confirmed', origin: 'app_manual', createdAt: new Date().toISOString() },
      ];
      setAppointments(mocks);
      localStorage.setItem('appointments', JSON.stringify(mocks));
      localStorage.setItem('appointments_version', MOCK_VERSION);
    } else {
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in pb-8 relative">
      ...
      <div className="flex items-center gap-4 mb-6 border-b border-stone-200 dark:border-stone-700 pb-1">
        ...
        >
          Link & Google {activeTab !== 'settings' && !isPremium && <Lock size={12} />}
        </button>
      </div>

      {activeTab === 'calendar' && (
        <div id="tour-agenda-view" className="lg:grid lg:grid-cols-12 lg:gap-6 flex flex-col gap-6">

          {/* LEFT COLUMN: Mini Calendar (Desktop) / Full Calendar (Mobile) */}
          <div className="lg:col-span-4 xl:col-span-3">
            <div className="bg-white dark:bg-stone-800 rounded-3xl shadow-sm border border-stone-100 dark:border-stone-700 overflow-hidden sticky top-0">
              <div className="p-4 lg:p-6 flex justify-between items-center border-b border-stone-100 dark:border-stone-700 bg-stone-50/50 dark:bg-stone-800/50">
                <h2 className="text-lg lg:text-xl font-display dark:text-stone-100 capitalize flex items-center gap-2">
                  <CalendarIcon className="text-gold-500" size={20} />
                  {currentMonth.toLocaleString('pt-BR', { month: 'short', year: 'numeric' })}
                </h2>
                <div className="flex gap-1">
                  <button onClick={() => changeMonth(-1)} className="p-1.5 hover:bg-white dark:hover:bg-stone-700 rounded-full text-stone-600 dark:text-stone-300 transition-all"><ChevronLeft size={20} /></button>
                  <button onClick={() => changeMonth(1)} className="p-1.5 hover:bg-white dark:hover:bg-stone-700 rounded-full text-stone-600 dark:text-stone-300 transition-all"><ChevronRight size={20} /></button>
                </div>
              </div>
              ...
              <div className="p-3 lg:p-4">
                <div className="grid grid-cols-7 mb-2">
                  {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
                    <div key={i} className="text-center text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase">{d}</div>
                  ))}
                </div>
                <div className="grid grid-cols-7 gap-1">
                  {calendarDays.map((day, idx) => {
                    if (!day) return <div key={idx} className="p-1 aspect-square"></div>;
                    const isToday = new Date().toDateString() === day.toDateString();
                    const isSel = isSelected(day);
                    const dayAppts = appointments.filter(a => a.date === formatDateKey(day));
                    ...
                    return (
                      <button
                        ...
                        className={`
                            relative p-1 rounded-lg flex flex-col items-center justify-center transition-all aspect-square text-sm font-medium
                            ${isSel ? 'bg-gold-500 text-stone-900 shadow-lg font-bold' : 'hover:bg-stone-100 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-300'}
                            ${isToday && !isSel ? 'ring-2 ring-gold-400 ring-inset' : ''}
                            ${apptCount >= 3 && !isSel ? 'bg-red-50 dark:bg-red-900/20' : ''}
                          `}
                      >
                        {day.getDate()}
                        {apptCount > 0 && (
                          <div className={`absolute bottom-0.5 flex gap-0.5`}>
                            {dayAppts.slice(0, Math.min(apptCount, 3)).map((_, i) => (
                              <div key={i} className={`w-1 h-1 rounded-full ${isSel ? 'bg-stone-900' : apptCount >= 3 ? 'bg-red-500' : 'bg-gold-500'}`}></div>
                            ))}
                          </div>
                        )}
                      </button>
              ...
              {/* Legend */}
              <div className="px-4 pb-4 flex gap-4 text-[10px] text-stone-500 justify-center">
                <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-gold-500"></div> Agendado</span>
                <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-red-500"></div> Dia Cheio</span>
              </div>
            </div>
          </div>

          {/* CENTER COLUMN: Appointments List */}
          <div className="lg:col-span-5 xl:col-span-6 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-2xl font-bold dark:text-stone-100">{selectedDate.getDate()} <span className="capitalize text-stone-500 font-normal">{selectedDate.toLocaleString('pt-BR', { month: 'long' })}</span></h3>
                <p className="text-xs text-stone-500 capitalize">{selectedDate.toLocaleString('pt-BR', { weekday: 'long' })}</p>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span className="bg-gold-100 dark:bg-gold-900/30 text-gold-700 dark:text-gold-400 px-3 py-1 rounded-full font-bold">{selectedDayAppointments.length} cliente{selectedDayAppointments.length !== 1 ? 's' : ''}</span>
              </div>
            </div>

            <div className="space-y-3 flex-1 overflow-y-auto max-h-[calc(100vh-300px)] lg:max-h-[calc(100vh-200px)] pr-2 custom-scrollbar">
              {selectedDayAppointments.length === 0 ? (
                <div className="text-center py-16 bg-stone-50 dark:bg-stone-800/50 rounded-3xl border-2 border-dashed border-stone-200 dark:border-stone-700">
                  ...
                  <p className="text-stone-500 font-medium">Nenhum agendamento neste dia</p>
                  <p className="text-xs text-stone-400 mt-1">Clique em "Marcar Cliente" para adicionar</p>
                </div>
              ) : (
                selectedDayAppointments.map(appt => (
                  <div key={appt.id} className="bg-white dark:bg-stone-800 p-5 rounded-2xl shadow-sm border border-stone-100 dark:border-stone-700 relative group hover:shadow-md hover:border-gold-200 dark:hover:border-gold-500/30 transition-all">
                    <div className="flex items-start gap-4">
                      {/* Time Badge */}
                      <div className="bg-stone-900 dark:bg-gold-500 text-white dark:text-stone-900 px-4 py-3 rounded-xl font-bold text-xl shadow-lg">
                        {appt.time}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="font-bold text-stone-800 dark:text-stone-100 text-lg truncate">{appt.clientName}</h4>
                          {appt.status === 'pending' && (
                            <span className="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">Pendente</span>
                          )}
                          {appt.status === 'confirmed' && (
                            <span className="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">Confirmado</span>
                          )}
                        </div>
                        <p className="text-sm text-gold-600 dark:text-gold-400 font-medium">{appt.service}</p>
                        {appt.notes && <p className="text-xs text-stone-400 mt-1 italic">"{appt.notes}"</p>}

                        <div className="flex items-center gap-3 mt-3">

                          {appt.origin === 'app_manual' && (
                            <span className="text-[10px] bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-400 px-2 py-1 rounded-lg font-bold">Manual</span>
                          )}
                        </div>
                      </div>

                      {/* Actions */}
                      <button onClick={() => deleteAppt(appt.id)} className="p-2 text-stone-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-3 flex flex-col gap-4">
            ...
            <div className="bg-stone-900 dark:bg-stone-700 text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <p className="text-gold-400 text-xs font-bold uppercase tracking-wider mb-1">Resumo do Dia</p>
                <h3 className="text-5xl font-display mb-2">{selectedDayAppointments.length}</h3>
                <p className="text-stone-300 text-sm">cliente{selectedDayAppointments.length !== 1 ? 's' : ''} agendado{selectedDayAppointments.length !== 1 ? 's' : ''}</p>
              </div>
              <button
                onClick={() => setShowForm(!showForm)}
                className="w-full mt-6 bg-gold-500 hover:bg-gold-400 text-stone-900 p-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 font-bold relative z-10"
              >
                {showForm ? <><X size={20} /> Cancelar</> : <><Plus size={20} /> Marcar Cliente</>}
              </button>
            </div>

            {/* New Appointment Form */}
            {showForm && (
              <div className="bg-white dark:bg-stone-800 p-5 rounded-2xl shadow-lg border border-gold-200 dark:border-gold-500/30 animate-fade-in">
                <h4 className="font-bold mb-4 dark:text-stone-100 flex items-center gap-2 text-sm"><Clock size={16} /> Novo Agendamento</h4>
                <div className="space-y-3">
                  <input
                    placeholder="Nome da Cliente"
                    className="w-full p-3 rounded-xl border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-stone-700 outline-none text-sm focus:border-gold-500"
                  ...
                  />
                  <button onClick={saveAppointment} className="w-full py-3 bg-stone-800 dark:bg-gold-500 text-white dark:text-stone-900 rounded-xl font-bold text-sm hover:opacity-90 transition-opacity">Confirmar</button>
                </div>
              </div>
            )}

            {/* Week Overview */}
            <div className="bg-white dark:bg-stone-800 p-5 rounded-2xl border border-stone-100 dark:border-stone-700">
              <h4 className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-3">Próximos 7 Dias</h4>
              <div className="space-y-2">
                {[0, 1, 2, 3, 4, 5, 6].map(offset => {
                  const day = new Date();
                  day.setDate(day.getDate() + offset);
                  const dayAppts = appointments.filter(a => a.date === formatDateKey(day));
                  const isSelectedDay = formatDateKey(day) === formatDateKey(selectedDate);
                  return (
                    <button
                      key={offset}
                      onClick={() => setSelectedDate(day)}
                      className={`w-full flex items-center justify-between p-2 rounded-lg text-sm transition-all ${isSelectedDay ? 'bg-gold-100 dark:bg-gold-900/30' : 'hover:bg-stone-50 dark:hover:bg-stone-700'}`}
                    >
                      <span className={`capitalize ${isSelectedDay ? 'font-bold text-gold-700 dark:text-gold-400' : 'text-stone-600 dark:text-stone-300'}`}>
                        {offset === 0 ? 'Hoje' : offset === 1 ? 'Amanhã' : day.toLocaleString('pt-BR', { weekday: 'short' })}
                      </span>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${dayAppts.length > 0 ? (dayAppts.length >= 3 ? 'bg-red-100 text-red-600' : 'bg-gold-100 text-gold-700') : 'bg-stone-100 text-stone-400 dark:bg-stone-700 dark:text-stone-500'}`}>
                        {dayAppts.length}
                      </span>
                    </button>
                  );
          ...
          <div className={`space-y-6 ${!isPremium ? 'opacity-50 pointer-events-none filter blur-sm select-none' : ''}`}>
            ...
            <div className="bg-white dark:bg-stone-800 p-6 rounded-3xl shadow-sm border border-stone-100 dark:border-stone-700">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium dark:text-stone-100 flex items-center gap-2"><LinkIcon size={20} className="text-gold-500" /> Link de Agendamento</h3>
                <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded-lg">ATIVO</span>
              </div>

              <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200 p-4 rounded-xl mb-6 text-sm flex gap-3">
                <RefreshCw className="shrink-0 mt-0.5" size={16} />
                <p>Este link permite que suas clientes agendem online. Se ativar a sincronização abaixo, todos os agendamentos cairão direto na sua <strong>Google Agenda</strong>!</p>
              </div>
