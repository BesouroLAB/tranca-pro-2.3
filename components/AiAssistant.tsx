...
const AiAssistant = ({ embedded = false }: { embedded?: boolean }) => {
  ...
  const getInitialMessage = () => {
    ...
    if (user?.communicationStyle === 'mentora') {
      return `Olá, ${greetingNoun}! Sou a Zuri. Estou aqui para dar suporte técnico e de gestão. Qual é a demanda de hoje? (Sinta-se à vontade para ajustar como conversamos clicando na engrenagem ali em cima).`;
    }
    // Default / Parceira
  ...
  const handleSend = async (text: string = input) => {
    ...
    if (user?.communicationStyle === 'formal') {
      ...
      styleInstruction = "Você é uma mentora sábia e acolhedora. Use linguagem carinhosa e solidária. Foque no emocional e na evolução profissional.";
    } else {
  ...
  return (
    <div className={`${heightClass} flex flex-col bg-white dark:bg-stone-800 rounded-2xl shadow-sm border border-stone-100 dark:border-stone-700 overflow-hidden relative`}>
      ...
      {/* Visual Timer Banner */}
      {user?.plan === 'free' && (
        <div className={`w-full py-1.5 px-4 text-xs font-bold text-center flex items-center justify-center gap-2 transition-colors ${timerStatus.status === 'blocked' ? 'bg-red-500 text-white' :
