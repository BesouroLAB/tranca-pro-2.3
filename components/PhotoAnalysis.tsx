...
import { Camera, Upload, Image as ImageIcon, Lock, ChevronDown, Clock, ChevronLeft, Sparkles, BookOpen, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/escaneamento')} className="w-12 h-12 bg-white dark:bg-stone-800 rounded-2xl flex items-center justify-center text-stone-500 border border-stone-100 dark:border-stone-700 hover:text-gold-500 transition-colors shadow-sm">
            <ChevronLeft size={24} />
          </button>
          <div>
            <h2 className="text-2xl font-display dark:text-stone-100 italic">Escaneamento de <span className="text-gold-500">Técnica</span></h2>
            <p className="text-stone-500 dark:text-stone-400 text-sm">Avaliação profissional da sua execução e fotografia.</p>
          </div>
        </div>
        {user?.plan === 'free' && (
        ...
        <div className="bg-white dark:bg-stone-800 p-6 rounded-2xl border border-stone-100 dark:border-stone-700 shadow-sm min-h-[400px] flex flex-col">
          ...
          {loading ? (
            ...
            <div className="prose prose-stone dark:prose-invert max-w-none text-sm animate-fade-in custom-scrollbar overflow-y-auto pr-2">
              ...
              {/* Zuri Recommendations from Aprenda */}
              <div className="mt-8 pt-6 border-t border-stone-100 dark:border-stone-700">
                <h4 className="flex items-center gap-2 font-bold text-stone-900 dark:text-stone-100 mb-4">
                  <Sparkles size={16} className="text-gold-500" /> Zuri Indica: Próximos Passos
                </h4>
                <div
                  className="bg-stone-50 dark:bg-stone-900/50 p-4 rounded-xl border border-stone-200 dark:border-stone-700 cursor-pointer hover:border-gold-500 transition-all group"
                  onClick={() => navigate('/aprender')}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-white dark:bg-stone-800 rounded-lg flex items-center justify-center text-gold-500 shadow-sm">
                        <BookOpen size={20} />
                      </div>
                      <div>
                        <p className="text-xs font-bold text-stone-800 dark:text-stone-200">
                          {selectedBraidType.includes('Nagô') ? 'Apostila: Divisões Geométricas' :
                            selectedBraidType.includes('Box') ? 'Vídeo: Acabamento Perfeito na Raiz' :
                              'Dica: Cronograma Capilar Pós-Trança'}
                        </p>
                        <p className="text-[10px] text-stone-500">Conteúdo recomendado para este estilo</p>
                      </div>
                    </div>
                    <ChevronRight size={16} className="text-stone-400 group-hover:text-gold-500 transition-colors" />
                  </div>
                </div>
              </div>
            </div>
