...
import { Check, ChevronRight, Award, Target, User, Share2, Star, ChevronLeft, Sparkles, BookOpen } from 'lucide-react';
...
const ProfessionalScan = () => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {techniques.map(t => (
          <button
            ...
            className={`p-3 rounded-xl border text-xs font-medium transition-all flex items-center justify-between ${data.skills.includes(t)
        ...
        {['Iniciante (Menos de 1 ano)', 'Intermediário (1 a 3 anos)', 'Experiente (3 a 5 anos)', 'Master (Mais de 5 anos)'].map((exp, idx) => {
          ...
          return (
            <button
              ...
              className={`w-full p-4 rounded-xl border text-left transition-all ${data.experience === exp
                ? 'bg-stone-800 text-white border-stone-800 dark:bg-gold-500 dark:text-stone-900 dark:border-gold-500'
      ...
      <div className="grid grid-cols-1 gap-3">
        {['Aumentar Faturamento', 'Organizar Agenda', 'Aprender Novas Técnicas', 'Marketing Digital'].map((goal) => (
          <button
            ...
            className={`w-full p-4 rounded-xl border text-center transition-all ${data.goal === goal
              ? 'bg-stone-800 text-white border-stone-800 dark:bg-gold-500 dark:text-stone-900 dark:border-gold-500'
              : 'bg-white dark:bg-stone-800 text-stone-600 dark:text-stone-300 border-stone-200 dark:border-stone-600 hover:bg-stone-50 dark:hover:bg-stone-700'
              }`}
          >
  ...
  const renderResult = () => (
    <div className="animate-fade-in">
      ...
      {/* Zuri Indica - Recommendations based on Scan */}
      <div className="mt-8 bg-gold-500/5 border border-gold-500/20 p-6 rounded-3xl animate-fade-in-up">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 bg-gold-500 rounded-2xl flex items-center justify-center text-stone-900 shadow-lg">
            <Sparkles size={20} />
          </div>
          <div>
            <h4 className="font-bold text-stone-900 dark:text-gold-400">Zuri Indica</h4>
            <p className="text-[10px] text-stone-500 dark:text-stone-400 font-bold uppercase tracking-widest">Dica de Estudo Personalizada</p>
          </div>
        </div>

        <div className="bg-white dark:bg-stone-800 p-4 rounded-2xl border border-stone-100 dark:border-stone-700 flex items-center justify-between group cursor-pointer hover:border-gold-500 transition-all" onClick={() => navigate('/aprender')}>
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-stone-100 dark:bg-stone-700 rounded-xl flex items-center justify-center text-gold-500">
              <BookOpen size={24} />
            </div>
            <div>
              <h5 className="font-bold text-sm text-stone-800 dark:text-stone-200">
                {data.goal === 'Aumentar Faturamento' ? 'Como Calcular o Valor da Sua Hora' :
                  data.goal === 'Marketing Digital' ? 'Marketing para Trancistas' :
                    data.skills.length < 5 ? 'Apostila: Divisões Geométricas' : 'Tabela de Preços Estratégica'}
              </h5>
              <p className="text-xs text-stone-500">Conteúdo recomendado para seu nível.</p>
            </div>
          </div>
          <ChevronRight size={18} className="text-stone-300 group-hover:text-gold-500 transition-colors" />
        </div>
      </div>
    ...
    <div className="max-w-xl mx-auto py-6">
      {step < 4 && (
        <div className="flex items-center gap-4 mb-8 px-4">
          <button onClick={() => navigate('/escaneamento')} className="w-10 h-10 bg-white dark:bg-stone-800 rounded-full flex items-center justify-center text-stone-500 border border-stone-100 dark:border-stone-700 hover:text-gold-500 transition-colors shadow-sm">
            <ChevronLeft size={20} />
          </button>
          <div className="flex-1">
            <h2 className="text-xl font-display dark:text-stone-100">Escaneamento de Carreira</h2>
            <p className="text-[10px] text-stone-400 font-bold uppercase tracking-widest">Mapeando seu Nível</p>
          </div>
          <div className="flex gap-1">
            {[1, 2, 3].map(i => (
              <div key={i} className={`h-1.5 w-6 rounded-full ${i <= step ? 'bg-gold-500' : 'bg-stone-200 dark:bg-stone-700'}`}></div>
            ))}
          </div>
        </div>
      )}
